    ...
    <div class="flex">
        ...
        <select id="inputSatuan" name="satuan_unit" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-r-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all cursor-pointer" required>
            ...
            <optgroup label="Cairan & Cat">
                ...
                <!-- VARIASI UI SECTION -->
                {{-- 2.5 VARIASI PRODUK (SHOPEE STYLE) --}}
                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm mb-6">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <div class="flex items-center gap-3">
                            <div class="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg"><i class="mdi mdi-format-list-bulleted-type leading-none"></i></div>
                            <div>
                                <h2 class="font-bold text-slate-800">Variasi Produk</h2>
                                <p class="text-[10px] font-bold text-slate-500 m-0">Tambah variasi seperti warna atau ukuran.</p>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" id="checkVariasi" name="has_variasi" value="1" class="sr-only peer toggle-checkbox">
                            <div class="w-11 h-6 bg-slate-300 peer-focus:outline-none rounded-full peer toggle-label after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                        </label>
                    </div>
                    
                    <div id="boxVariasi" class="hidden p-6 space-y-6">
                        <div id="variationBuilder" class="space-y-4">
                            <!-- Variasi 1 -->
                            <div class="border border-slate-200 rounded-xl p-4 bg-slate-50">
                                <div class="flex items-center justify-between mb-3">
                                    <input type="text" name="variasi_nama[0]" id="namaVar0" class="bg-white border border-slate-300 rounded px-3 py-1.5 text-sm font-bold w-1/3" placeholder="Contoh: Warna" value="Warna" onkeyup="generateMatrix()">
                                </div>
                                <div id="opsiList0" class="flex flex-wrap gap-2">
                                    <!-- Container Opsi -->
                                    <button type="button" onclick="addOpsi(0)" class="text-indigo-600 text-sm font-bold px-3 py-1 border border-dashed border-indigo-300 rounded hover:bg-indigo-50 btn-add-opsi">+ Tambah Opsi</button>
                                </div>
                            </div>
                            
                            <!-- Tombol Tambah Variasi 2 -->
                            <button type="button" id="btnTambahVariasi2" class="text-indigo-600 text-sm font-bold w-full py-3 border border-dashed border-indigo-300 rounded-xl hover:bg-indigo-50" onclick="toggleVariasi2(true)">+ Tambah Variasi 2 (Opsional)</button>

                            <!-- Variasi 2 -->
                            <div id="boxVariasi2" class="hidden border border-slate-200 rounded-xl p-4 bg-slate-50">
                                <div class="flex items-center justify-between mb-3">
                                    <input type="text" name="variasi_nama[1]" id="namaVar1" class="bg-white border border-slate-300 rounded px-3 py-1.5 text-sm font-bold w-1/3" placeholder="Contoh: Ukuran" value="Ukuran" onkeyup="generateMatrix()">
                                    <button type="button" id="btnHapusVariasi2" class="text-red-500 text-sm font-bold" onclick="toggleVariasi2(false)"><i class="mdi mdi-delete"></i> Hapus</button>
                                </div>
                                <div id="opsiList1" class="flex flex-wrap gap-2">
                                    <!-- Container Opsi -->
                                    <button type="button" onclick="addOpsi(1)" class="text-indigo-600 text-sm font-bold px-3 py-1 border border-dashed border-indigo-300 rounded hover:bg-indigo-50 btn-add-opsi">+ Tambah Opsi</button>
                                </div>
                            </div>
                        </div>

                        <!-- Terapkan Massal -->
                        <div id="massApplyBox" class="hidden flex gap-3 mt-4 items-end bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                            <div>
                                <label class="text-xs font-bold text-slate-600">Harga Semua</label>
                                <input type="number" id="massHarga" class="w-32 bg-white border border-slate-300 rounded px-2 py-1 text-sm mt-1" placeholder="Rp 0">
                            </div>
                            <div>
                                <label class="text-xs font-bold text-slate-600">Stok Semua</label>
                                <input type="number" id="massStok" class="w-24 bg-white border border-slate-300 rounded px-2 py-1 text-sm mt-1" placeholder="0">
                            </div>
                            <button type="button" onclick="applyMass()" class="bg-indigo-600 text-white text-sm font-bold px-4 py-1.5 rounded hover:bg-indigo-700">Terapkan Ke Semua</button>
                        </div>

                        <!-- Matrix Table -->
                        <div id="variationMatrixWrapper" class="hidden mt-2 overflow-x-auto">
                            <h3 class="font-bold text-slate-800 mb-3 text-sm">Daftar Variasi (SKU)</h3>
                            <table class="w-full text-left text-sm border-collapse border border-slate-200">
                                <thead class="bg-slate-50">
                                    <tr>
                                        <th class="border border-slate-200 px-4 py-2" id="thVar1">Warna</th>
                                        <th class="border border-slate-200 px-4 py-2 hidden" id="thVar2">Ukuran</th>
                                        <th class="border border-slate-200 px-4 py-2 w-40">Harga <span class="text-red-500">*</span></th>
                                        <th class="border border-slate-200 px-4 py-2 w-32">Stok <span class="text-red-500">*</span></th>
                                        <th class="border border-slate-200 px-4 py-2 w-48">Kode Variasi</th>
                                    </tr>
                                </thead>
                                <tbody id="matrixBody">
    ...
    $(document).ready(function() {
        ...
        function calcPrice() {
            ...
            $('#hpPrice').text(formatRupiah(final));
                ...
                // ==============================================================
        // LOGIKA VARIASI PRODUK (SHOPEE STYLE)
        // ==============================================================
        const checkVariasi = $('#checkVariasi');
        const boxVariasi = $('#boxVariasi');
        let hasVariasi2 = false;

        checkVariasi.on('change', function() {
            if (this.checked) {
                boxVariasi.slideDown(200);
                // Disable global price/stock
                $('#viewHarga').prop('disabled', true).prop('required', false).addClass('bg-slate-200 cursor-not-allowed').val('');
                $('#rawHarga').val('');
                $('input[name="stok"]').prop('disabled', true).prop('required', false).addClass('bg-slate-200 cursor-not-allowed').val('');
                if($('.opsi-input').length === 0) {
                    addOpsi(0); // auto add 1 option if empty
                }
                generateMatrix();
            } else {
                boxVariasi.slideUp(200);
                $('#viewHarga').prop('disabled', false).prop('required', true).removeClass('bg-slate-200 cursor-not-allowed');
                $('input[name="stok"]').prop('disabled', false).prop('required', true).removeClass('bg-slate-200 cursor-not-allowed');
            }
        });

        window.toggleVariasi2 = function(show) {
            hasVariasi2 = show;
            if (show) {
                $('#boxVariasi2').slideDown(200);
                $('#btnTambahVariasi2').hide();
                $('#thVar2').removeClass('hidden');
                if($('#opsiList1 .opsi-input').length === 0) addOpsi(1);
            } else {
                $('#boxVariasi2').slideUp(200);
                $('#btnTambahVariasi2').show();
                $('#thVar2').addClass('hidden');
                // Clear opsi 2
                $('#opsiList1 .opsi-item').remove();
            }
            generateMatrix();
        }

        window.addOpsi = function(level) {
            let list = $('#opsiList' + level);
            let btn = list.find('.btn-add-opsi');
            let itemHtml = `
                <div class="flex items-center gap-1 bg-white border border-slate-300 rounded px-2 py-1 opsi-item">
                    <input type="text" name="variasi_opsi[${level}][]" class="outline-none text-sm w-24 opsi-input" placeholder="Opsi Baru" onkeyup="generateMatrix()">
                    <button type="button" class="text-slate-400 hover:text-red-500" onclick="this.parentElement.remove(); generateMatrix()"><i class="mdi mdi-close"></i></button>
                </div>
            `;
            $(itemHtml).insertBefore(btn);
            generateMatrix();
        }

        window.applyMass = function() {
            let mh = $('#massHarga').val();
            let ms = $('#massStok').val();
            if(mh) {
                $('.matrix-harga').val(mh);
                $('.matrix-harga-view').val(formatRupiah(mh).replace('Rp ', ''));
            }
            if(ms) $('.matrix-stok').val(ms);
        }

        window.formatMatrixHarga = function(el) {
            let val = $(el).val().replace(/[^0-9]/g, '');
            $(el).siblings('.matrix-harga').val(val);
            $(el).val(formatRupiah(val).replace('Rp ', ''));
        }

        window.generateMatrix = function() {
            if(!checkVariasi.is(':checked')) return;

            let nama1 = $('#namaVar0').val() || 'Variasi 1';
            let nama2 = $('#namaVar1').val() || 'Variasi 2';
            $('#thVar1').text(nama1);
            $('#thVar2').text(nama2);

            let opsi1 = [];
            $('#opsiList0 .opsi-input').each(function() {
                let v = $(this).val().trim();
                if(v) opsi1.push(v);
            });
            ...
            let opsi2 = [];
            if(hasVariasi2) {
                $('#opsiList1 .opsi-input').each(function() {
                    let v = $(this).val().trim();
                    if(v) opsi2.push(v);
                });
            }
            ...
            if(opsi1.length === 0) {
                $('#variationMatrixWrapper, #massApplyBox').addClass('hidden');
                return;
            }

            $('#variationMatrixWrapper, #massApplyBox').removeClass('hidden');

            let tbody = $('#matrixBody');
            
            // Backup existing values (to prevent overriding when typing)
            let existingValues = {};
            tbody.find('tr').each(function() {
                let key = $(this).data('key');
                let h = $(this).find('.matrix-harga').val();
                let s = $(this).find('.matrix-stok').val();
                let k = $(this).find('.matrix-sku').val();
                existingValues[key] = { h, s, k };
            });

            tbody.empty();

            if(!hasVariasi2 || opsi2.length === 0) {
                opsi1.forEach((o1, i) => {
                    let key = o1;
                    let ex = existingValues[key] || { h:'', s:'', k:'' };
                    let vH = ex.h ? formatRupiah(ex.h).replace('Rp ', '') : '';
                    let tr = `
                        <tr data-key="${key}" class="border-b border-slate-100 hover:bg-slate-50">
                            <td class="px-4 py-3 font-medium text-slate-700">${o1}
                                <input type="hidden" name="sku_opsi1[]" value="${o1}">
                                <input type="hidden" name="sku_opsi2[]" value="">
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex">
                                    <span class="inline-flex items-center px-2 bg-slate-100 border border-r-0 border-slate-300 rounded-l text-xs">Rp</span>
                                    <input type="text" class="matrix-harga-view w-24 border border-slate-300 rounded-r px-2 py-1 text-sm outline-none focus:border-indigo-500" value="${vH}" onkeyup="formatMatrixHarga(this)" required>
                                    <input type="hidden" name="sku_harga[]" class="matrix-harga" value="${ex.h}">
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <input type="number" name="sku_stok[]" class="matrix-stok w-20 border border-slate-300 rounded px-2 py-1 text-sm outline-none focus:border-indigo-500" value="${ex.s}" required min="0">
                            </td>
                            <td class="px-4 py-3">
                                <input type="text" name="sku_kode[]" class="matrix-sku w-full border border-slate-300 rounded px-2 py-1 text-sm outline-none focus:border-indigo-500 font-mono" value="${ex.k}">
                            </td>
                        </tr>
                    `;
                    tbody.append(tr);
                ...
                opsi1.forEach((o1, i) => {
                    opsi2.forEach((o2, j) => {
                        ...
                        let ex = existingValues[key] || { h:'', s:'', k:'' };
                        let vH = ex.h ? formatRupiah(ex.h).replace('Rp ', '') : '';
                        let td1 = j === 0 ? `<td class="px-4 py-3 font-medium text-slate-700 border-r border-slate-100" rowspan="${opsi2.length}">${o1}</td>` : '';
                        
                        let tr = `
                            <tr data-key="${key}" class="border-b border-slate-100 hover:bg-slate-50">
                                ${td1}
                                <td class="px-4 py-3 font-medium text-slate-700">${o2}
                                    <input type="hidden" name="sku_opsi1[]" value="${o1}">
                                    <input type="hidden" name="sku_opsi2[]" value="${o2}">
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex">
                                        <span class="inline-flex items-center px-2 bg-slate-100 border border-r-0 border-slate-300 rounded-l text-xs">Rp</span>
